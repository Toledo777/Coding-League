...
router.post('/user/create', express.json(), async(req, res) => {
	...
	if (req.body) {
		...
		res.status(400).json({title: 'ERROR: No user data in body'});
	}
 ...
 * uses the default _id primary key generated by mongo
 */
router.put('/user/update', express.json(), async (req, res) => {
	...
	if (req.body.id) {
		...
		const response = await user.updateOne({_id: req.body.id}, userData);
		if (response != undefined) {
			res.json(response);
		}
	...
	// missing id parameter
		...
		res.status(400).json({ title: 'ERROR: Missing ID parameter' });
	}
